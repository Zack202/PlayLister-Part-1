...
export default class PlaylisterController {
    ...
    */
    initModalHandlers() {
        ...
        this.model.addDeleteSongTransaction(mIndex);
        this.model.whenmodalclose();
        ...
        let confirmEditSongList = document.getElementById("edit-song-confirm-button");
        confirmEditSongList.onclick = (event) => {
            let name = document.getElementById("tInput").value;
            let artist = document.getElementById("aInput").value;
            let youtubeid = document.getElementById("yInput").value;
            this.model.whenmodalclose();
            this.model.addEditSongTransaction(mIndex,name,artist,youtubeid);

          //  this.model.toggleConfirmDialogOpen();
            let editSongsModal = document.getElementById("edit-songs-modal");
                editSongsModal.classList.remove("is-visible");
        }

        let cancelEditSongList = document.getElementById("edit-song-cancel-button");
        cancelEditSongList.onclick = (event) => {
            //this.model.toggleConfirmDialogOpen();
            this.model.whenmodalclose();
            let editSongsModal = document.getElementById("edit-songs-modal");
                editSongsModal.classList.remove("is-visible");
    ...
    registerItemHandlers() {
        ...
        for (let i = 0; i < this.model.getPlaylistSize(); i++) {
            ...
            card.ondblclick = (event) => {
                let editSongsModal = document.getElementById("edit-songs-modal");
                editSongsModal.classList.add("is-visible");
                this.model.whenmodalopen();
              //  this.model.toggleConfirmDialogOpen();
                mIndex = Number.parseInt(event.target.id.split("-")[2]) - 1;
 
            }
            document.getElementById("delete-song-" + (i+1)).onmousedown = (event) => {
                mIndex = Number.parseInt(event.target.id.split("-")[2]) - 1;

                let listName = this.model.currentList.songs[mIndex].title;
                let deleteSpan = document.getElementById("delete-song-span");
                deleteSpan.innerHTML = "";
                deleteSpan.appendChild(document.createTextNode(listName));

                let editSongsModal = document.getElementById("delete-song-modal");
                editSongsModal.classList.add("is-visible");
                this,this.model.whenmodalopen();
            }
        }
