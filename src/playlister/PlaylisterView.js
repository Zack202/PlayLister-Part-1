    ...
    */
    refreshPlaylist(playlist) {
        ...
        for (let i = 0; i < playlist.songs.length; i++) {
            ...
            // Create counter for songs
            let textBeg = document.createTextNode(j + ". ");

            // Create hyperlink for song title
            let a = document.createElement("a")
            let textsong = document.createTextNode(song.title+" by " + song.artist);
            a.appendChild(textsong);
            a.href = "https://www.youtube.com/watch?v=" + song.youTubeId;
            
            // Create song artist for song
            

            let trashBox = document.createElement("input");
            trashBox.classList.add("song-card-button");
            trashBox.id = "delete-song-" + (i+1);
            trashBox.type = "button";
            trashBox.value = "X";

            // Combine all parts
            itemDiv.appendChild(textBeg);
            itemDiv.appendChild(a);
            itemDiv.appendChild(trashBox);
